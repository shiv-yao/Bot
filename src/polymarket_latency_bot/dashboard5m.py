...
<article class="card"><div class="mono"><a href="/ai/status">/ai/status</a> · <a href="/risk/profile">/risk/profile</a> · <a href="/monitor">/monitor</a> · <a href="/diagnostics">/diagnostics</a> · <a href="/performance">/performance</a></div></article>
</section></main><script>
const $=id=>document.getElementById(id);const pct=x=>Number.isFinite(Number(x))?`${(Number(x)*100).toFixed(2)}%`:'—';const usd=x=>Number.isFinite(Number(x))?`$${Number(x).toFixed(4)}`:'—';const row=(a,b)=>`<div class="row"><span>${a}</span><strong>${b}</strong></div>`;async function refresh(){const [s,a]=await Promise.all([fetch('/state',{cache:'no-store'}).then(r=>r.json()),fetch('/ai/status',{cache:'no-store'}).then(r=>r.json())]);const p=s.paper_portfolio||{},sum=p.summary||{},m=s.current_market||{},prices=s.btc_prices_tail||[],last=prices.length?prices[prices.length-1]:null;const ai=a.ai||{},r=a.risk_profile||{};$('status').textContent=s.market_discovery_status==='ready'?'SYSTEM ACTIVE':'WAITING';$('btc').textContent=last?`$${Number(last[1]).toLocaleString(undefined,{maximumFractionDigits:2})}`:'—';$('direction').textContent=ai.direction||'WAIT';$('prob').textContent=`上漲機率 ${pct(ai.fair_probability_up)} · 信心 ${pct(ai.confidence)}`;$('pnl').textContent=usd(sum.realized_pnl||0);$('winrate').textContent=pct(sum.win_rate||0);$('record').textContent=`勝 ${sum.wins||0} · 負 ${sum.losses||0} · 平 ${sum.flat||0}`;$('marketStatus').textContent=s.market_discovery_status||'—';$('slug').textContent=m.slug||'—';$('question').textContent=m.question||'—';$('profile').innerHTML=row('單筆權益比例',pct(r.effective_max_order_equity_fraction))+row('日損線',pct(r.effective_max_daily_loss_fraction))+row('總曝險',usd(r.effective_max_open_notional_usd))+row('冷卻',`${r.signal_cooldown_ms||0} ms`)+row('策略評估',`${r.strategy_evaluation_interval_ms||0} ms`)+row('Workers',r.execution_workers||0)+row('Queue',r.max_queue_size||0)}refresh();setInterval(refresh,2000)
</script></body></html>
"""
